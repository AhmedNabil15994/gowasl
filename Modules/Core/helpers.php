...
if (!function_exists("changeQR")) {
    function changeQR($data){
        $save=false;

        $source = imagecreatefromstring((file_get_contents($data['qr'])));  # QR-Code
        $logo =  imagecreatefromstring((file_get_contents($data['logo'])));    # Overlay
        $filepath = public_path('/uploads/qrImages/'.$data['fileName']);
