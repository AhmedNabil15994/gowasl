...
abstract class RouteServiceProvider extends ServiceProvider
{
    ...
    {
        return '\Modules\\' . $this->module_name . '\\Http\Controllers\Vendor';
    }
    ...
    protected function getVendorRoutes(): array
    {
        $routes = [];

        if(count($this->vendor_routes)){

            foreach ($this->vendor_routes as $file){

                $routes[module_path($this->module_name, 'Routes/vendor') .'/'.$file] = $this->vendorGroups();
            }
        }
...
//       TODO $this->loadRoutesFrom(DIR.'/../routes/web.php');
        ...
        $router->group(['namespace' => $this->getVendorNameSpace()], function (Router $router) {
            $this->loadVendorRoutes($router);
        });
    ...
    private function loadVendorRoutes(Router $router)
    {
        $vendorRoutes = (array)$this->getVendorRoutes();
        $this->requireRoutesFiles($router,$vendorRoutes);
    }
