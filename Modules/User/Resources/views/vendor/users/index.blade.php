...
@section('title', __('user::vendor.users.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('user::vendor.users.index.title')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('vendor.users.create')) }}"
                        ...
                        <table class="table table-striped table-bordered table-hover"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('user::vendor.users.datatable.image')}}</th>
                                    <th>{{__('user::vendor.users.datatable.name')}}</th>
                                    <th>{{__('user::vendor.users.datatable.email')}}</th>
                                    <th>{{__('user::vendor.users.datatable.mobile')}}</th>
                                    <th>{{__('user::vendor.users.datatable.created_at')}}</th>
                                    <th>{{__('user::vendor.users.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit"
                                ...
                                onclick="deleteAllChecked('{{ url(route('vendor.users.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('vendor.users.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
...
responsivePriority: 1,              width: '13%',
              title: '{{__('user::vendor.users.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("vendor.users.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("vendor.users.destroy", ":id") }}';
