...
class RoleRepository extends CrudRepository
{
    ...
    public function getAllUsersRoles($order = 'id', $sort = 'desc')
    {
        $roles = $this->model->where('id','!=',1)->whereHas('permissions', function($query){
            $query->where('name','seller_access');
        })->orderBy($order, $sort)->get();
        return $roles;
    }
     public function getAllOwnerRoles($order = 'id', $sort = 'desc')
    {
        $roles = $this->model->whereHas('permissions', function($query){
                    $query->where('name','owner_access');
                 })->orderBy($order, $sort)->get();
        return $roles;
    }
